...
for run in range(1,11,1):
    ...
    thinned_sim_HI_fileName = "/Users/charles/Documents/researchData/clustering/simulations/rep" + str(run)+"/CHANGE_FILENAME.txt"
    fw = open(thinned_sim_HI_fileName, 'w')
    ...
    for line in f:
        ...
        if(serumNumber <= 2):
            if(distCluster[virusCluster][serumCluster] <= 3):
                if(random.uniform(0,1) < 0.2):
                    fw.write(line)
        elif(serumNumber <= 5 & serumNumber >2):
            if(distCluster[virusCluster][serumCluster] <= 2):
                if(random.uniform(0,1) < 0.13):
                    fw.write(line)
        else:
            if(distCluster[virusCluster][serumCluster] <= 5):
                if(random.uniform(0,1) < 0.3):
                    fw.write(line)
    fw.close()
    f.close()
