...
Posterior distributions were approximated using Markov chain Monte Carlo with model-specific proposals to address the challenge of efficient mixing. 
...
In A/H3N2, our method identified clusters with high confidence.  % and estimated the rate of antigenic transitions to be XXX events per lineage per year.
...
%Trevor: what's actually interesting?
...
%Trevor: please add one or two sentences of main contributions
...
%    p( I, \mu , \pi,Y , \beta | H, T, d) \propto  p(H| X(T, I, \mu), Y ) p(I |\pi) p(\pi)  p(\mu) p(Y|  \beta, d)   p(\beta)
\end{equation}
...
%Trevor can decide what to write.
%Not using this default kappa anymore.. and we don't count by at least 5 observed viruses = cluster now..
...
%	\caption{
	%Need to fill in the text once the Figure is available. Maybe Trevor can write this more accurately.
	
	%\textbf{Inferred antigenic clusters on the A/H3N2 phylogeny.}
...
%\textbf{Inferred antigenic clusters on the A/H1N1 phylogeny.}
% The coloring corresponds to the MCMC sample that gives the highest posterior density. 
  % The area of each node is proportional to the probability that the node is active. 
 %The root node has probability = 1. 
...
%Note: this is outdated: we now use kappa = 0.1
...
Last, the ESS reported in Tracer \cite{tracer-website} also suggests that the chain mixed adequately.
...
%Among the worst performing $\mu_{i2}$, we observe that their distribution are bimodal about 0, and they have lower ESS because it is relatively much more difficult to cross from 1 hill to another hill across 0.
%This situation arises when there is not much data to constrain the inference of $\mu_{i2}$ to either go up or down by the same distance about 0.
%In general, we do not believe that this is a problem for our analysis because the likelihood is only affected by the relative distance between viruses and sera but not the absolute sign of $\mu_{i2}$, and we only concern with the Euclidean distance between viruses and sera.
...
\begin{equation}
 p(H|X, Y, \mdssd^2) = \prod_{(i,j) \in \cal I} f(H_{ij} |  X_i  = x_i, Y_j = y_j,\mdssd^2) 
\end{equation}
...
The MCMC algorithm generates dependent random samples that collectively approximate the posterior distribution.
...
Heatmap is a very useful summary of clustering because it ultimately captures the probability that each pair of viruses share the same clusters, hence visually presenting the major clustering patterns among all viruses, even when the exact transition branches cannot be inferred with high confidence. %a sentence about why heatmap is highly appropriate 
Fourth, we performed convergence analysis by (1)inspecting convergence diagnostic plots of MCMC outputs generated by Tracer v1.6, (2)comparing across replicates the probabilities that each node is active, which are the main parameters of interest, and (3)computing the Gelman-Rubin statistics on these parameters. 
