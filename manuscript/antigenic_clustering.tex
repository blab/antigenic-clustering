...
%%%\author[3]{Gabriela B. Cybis}
\author[4]{Andrew Rambaut}
\author[1]{Trevor Bedford}

\affil[1]{Vaccine and Infectious Disease Division, Fred Hutchinson Cancer Research Center, Seattle, WA}
\affil[4]{Institute of Evolutionary Biology, University of Edinburgh, Ashworth Laboratories, Edinburgh, UK and Fogarty International Center, National Institutes of Health, Bethesda, MD, USA}
%%%\affil[2]{Departments of Biomatematics and Human Genetics, David Geffen School of Medicine and Department of Biostatistics, Fielding School of Public Health, University of California, Los Angeles, California, U.S.A}
%%%\affil[3]{Department of Statistics, Federal University of Rio Grande do Sul, Porto Alegre, RS, Brazil}
...
%Completed
%\begin{itemize}
 %%% \item DONE - Finalize model, run main analyses
%%%  \item DONE - Compile results into Tables and Figures
  %%%  \item  DONE - Abstract - Due date: Feb 2nd, 2015 (for JSM 2015) [1200 word limit] 
	%%%http://www.amstat.org/meetings/jsm/2015/abstracts.cfm%%%
  %%% \item DONE - Organize Method section 
  %%%  \item DONE Prepare outputs for Trevor to make the graphics
  %  \item DONE - Performed simulations
       %\item DONE - Draft the Introduction
%       \item DONE - Draft the New Approaches section 
%    \item DONE - Draft the Method section -revision on the existing content and write experimental plan
% \item DONE -   \item Draft the Results section (Organize)
%  \item DONE - Write the discussion
  %\item Revise paper
  %\item send to Trevor
%\end{itemize}
...
%To-do
%\begin{itemize}
%  \item  upload MCMC runs online  
       %%% eg. the data collection, infer phylogeny section of the methods, make the plot%%%

  %\item make super major revision
  %\item invite collaborators to read
%\end{itemize}       
...
%%%  can have 250 words - still can add a sentence or two.%%%
Each year, influenza viruses cause over 250,000 deaths. 
...
Here, we extended this method to enable systematic clustering of viruses into discrete antigenic phenotypes. 
We explicitly leveraged information from virus phylogeny to relate viral evolution to the emergence of major antigenic phenotypes. 
Our model extended a recently introduced Bayesian multidimensional scaling technique for antigenic cartography, thereby capturing uncertainty in clustering and cartographic location. 
In this framework, we used the Bayesian Stochastic Search Variable Selection to probabilistically infer branches associated with antigenic transitions. 
We developed model-specific proposals to address the challenge of mixing when computing the posterior estimates using the Markov chain Monte Carlo sampling technique. 
In our considered setting, results from simulations suggest that our approach can detect antigenic clusters with high sensitivity ($\sim$90\%), even when the percentage of missing data is high (90\%).
We analyzed several decades of historical A/H1N1 and A/H3N2 antigenic data and successfully inferred circulating antigenic phenotypes. 
In A/H3N2, our method inferred almost all identified transitions from 1968-2004(2?) from a previous work with high confidence and inferred additional antigenic transitions until 2010.
Our inference also suggested the branches leading to antigenic transitions and estimated other antigenic parameters.
%what else are important from the paper that I missed?
We believe that this method may have utility in facilitating surveillance of antigenic phenotypes and therefore vaccine strain selection for antigenically variable pathogens. %I think we should back up this statement in the text if we want to put this sentence in
...
%Question: do I need to go into explicit details about how we measure antigenic variation in the introduction? Why is it important, or is it not?
...
For the remaining TX77 strain previously identified, our clustering method also detected some evidence that it was antigenically different from its neighboring clusters, although the lack of certainty is likely due to small sample size, with only TX77 and potentially A/Bilthoven/2271/1976 showing the most evidence of co-clustering (Figure \ref{TX77heatmap}). %co-clustering = right word?
In fact, the TX77 strain also shows evidence that it is both associated with the earlier (EN72) and later (BK79) clusters .
% giving some evidence to "antigenic continuum", or the lack of definite placement of a strain to one cluster.  %say here?
...
In addition, we demonstrated that the BSSVS framework, since its introduction to phylodynamics \cite{lemey_bayesian_2009, drummond_bayesian_2010}, can also carry the task of clustering virus strains on a phylogeny by data from a high dimensional trait.  % is it okay to  keep it here? or does this sentence belong to somewhere else?
...
Heatmap is a very useful summary of clustering because it ultimately capture the probability of each pair of virus sharing the same cluster and visually presents co-clustering in the majority of points, even when the exact transition branch cannot be inferred with high confidence. %a sentence about why heatmap is highly appropriate 
